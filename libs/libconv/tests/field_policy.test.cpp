...
namespace quick_dra::testing {
	...
	namespace getters {
		struct name {
			...
			inline std::optional<std::string>& operator()(person_type& person) const noexcept { return person.name; }
		};

		struct species {
			using person_type = creature;
			inline std::optional<std::string>& operator()(person_type& person) const noexcept { return person.species; }
		};

		struct age {
			using person_type = creature;
			inline std::optional<unsigned>& operator()(person_type& person) const noexcept { return person.age; }
		};
	}  // namespace getters

	static constexpr auto lab_sel = "Creature species"_label / getters::species{};
	static constexpr auto species = lab_sel / builtin::policies::always_valid<std::string>;
	static constexpr auto name = "Name"_label / getters::name{} / builtin::policies::always_valid<std::string>;
	static constexpr auto age =
	    "Age"_label / getters::age{} / [](std::string&& val, std::optional<unsigned>& dst, bool) {
		    unsigned result{};
	...
	using policy_type = field_policy<A, S, policy_builder::validator_function<A>>;

	template <typename A, typename S>
	using field_answer_type = interactive::field_answer<A, policy_type<A, S>>;

	template <typename FieldPolicy, typename Person>
	bool validate_and_update(FieldPolicy const& policy, Person& dst, std::string&& value) {
		return policy.get_validator()(std::move(value), policy.select(dst), false);
	}
	...
	TEST(field_policy, builder) {
		...
		EXPECT_TRUE(
		    (std::same_as<decltype(lab_sel), policy_builder::label_selector<std::string, getters::species> const>));
		EXPECT_TRUE((std::same_as<decltype(species), policy_type<std::string, getters::species> const>));
	...
	TEST(field_policy, get_field_answer) {
		...
		EXPECT_TRUE((std::same_as<decltype(answer), field_answer_type<std::string, getters::species> const>));
...
#define EXPECT_POLICY_VALID(TARGET, POLICY, VALUE) EXPECT_POLICY(TARGET, POLICY, VALUE, TRUE, VALUE##sv)
#define EXPECT_POLICY_VALID_(TARGET, POLICY, VALUE, EXPECTED) EXPECT_POLICY(TARGET, POLICY, VALUE, TRUE, EXPECTED)
#define EXPECT_POLICY_INVALID(TARGET, POLICY, VALUE) EXPECT_POLICY(TARGET, POLICY, VALUE, FALSE, std::nullopt)
		...
		EXPECT_POLICY_INVALID(insured, builtin::policies::passport, "AA1000000");
		EXPECT_POLICY_VALID(insured, builtin::policies::social_id, "26211012346");
	...
	TEST(field_policy, enums) {
		...
		static_assert(interactive::details::Enumerator<decltype(social_id_item)>);
		static_assert(interactive::details::Enumerator<decltype(passport_item)>);
		...
		{
			auto kind_enum = builtin::policies::kind.get_enum_field(get_enum_item(builtin::policies::social_id),
...

		{
			...
			EXPECT_TRUE(result);
			EXPECT_EQ(conv.dst.kind, "2"s);
			EXPECT_EQ(log, "\033[0;36mDocument kind\033[0;90m [P - PESEL, [2] - Passport]\033[m> "sv);
		}
