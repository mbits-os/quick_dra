...
namespace quick_dra::builtin::testing {
	namespace {
		...
		void skip_ws(std::string_view::iterator& it, std::string_view::iterator const& end) {
			while (it != end && std::isspace(static_cast<unsigned char>(*it)))
				++it;
		}

		std::string quoted(std::string_view::iterator& it, std::string_view::iterator const& end, char quote) {
			std::string result{};
			auto start = it;
		...
		std::string escaped(std::string_view::iterator& it, std::string_view::iterator const& end) {
			std::string result{};
			auto escaped{false};
			while (it != end) {
				auto const c = *it;
		...
		std::string get_argument(std::string_view::iterator& it, std::string_view::iterator const& end) {
			std::string result;
			while (it != end && !std::isspace(static_cast<unsigned char>(*it))) {
				if (*it == '\'') {
					++it;
		...
		[[noreturn]] void throw_on_exit(int code) { throw exit_code{.value = code}; }

		closed_run run(std::string_view arguments) {
			auto args = split_args(arguments);
			std::vector<char*> argv{};
			argv.reserve(args.size() + 1);
			std::transform(args.begin(), args.end(), std::back_inserter(argv), [](auto& s) { return s.data(); });
			...
			try {
				result.returncode = tool({"qdra"sv, args::arglist{static_cast<unsigned>(args.size()), argv.data()}});
		...
		inline std::filesystem::path _data_dir() {
			...
			auto const root = platform::exec_dir().parent_path().parent_path().parent_path();
		...
		void expect_test(runnable_testcase const& expected) {
			...
			if (!expected.config_name.empty() || !expected.config.empty()) {
				...
				if (expected.mode != perms::none) {
					std::filesystem::permissions(output, expected.mode, std::filesystem::perm_options::replace);
				}
			}
			...
			if (expected.writes) {
				...
				if (file.cmp.empty()) {
					std::error_code ec{};
					EXPECT_TRUE(std::filesystem::is_regular_file(file.name, ec)) << "Wrote: " << file.name;
					EXPECT_TRUE(ec) << "Wrote: " << file.name;
				} else {
					auto const cmp_path = data_dir() / file.cmp;
					auto const actual_content = patched(file_contents(file.name));
					std::error_code ec{};
					if (!std::filesystem::exists(cmp_path, ec)) {
						std::ofstream out{cmp_path, std::ios::out | std::ios::binary};
						out.write(actual_content.data(), static_cast<std::streamsize>(actual_content.size()));
					} else {
						auto const expected_content = file_contents(data_dir() / file.cmp);
						EXPECT_EQ(actual_content, expected_content) << fmt::to_string(data_dir() / file.cmp);
					};
				}
			}
		}
