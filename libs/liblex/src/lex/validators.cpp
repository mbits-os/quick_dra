...
namespace quick_dra::detail {
	namespace {
		constexpr auto tax_id_checker =
		    checker::mask<"000000000?">.with<6, 5, 7, 2, 3, 4, 5, 6, 7, 0>().postproc([](auto sum) {
		...
		constexpr auto id_card_checker =
		    checker::mask<"AAA?00000">.with<7, 3, 1, 0, 7, 3, 1, 7, 3>().postproc([](auto sum) { return sum % 10; });

		constexpr auto pl_passport_checker =
		    checker::mask<"AA?000000">.with<7, 3, 0, 1, 7, 3, 1, 7, 3>().postproc([](auto sum) { return sum % 10; });
	...
	std::chrono::year_month_day social_id::get_birthday(std::string_view social_id) noexcept {
		...
		auto const month_and_century = int_parse<unsigned>(social_id.substr(2, 2));
		...
		auto const century = century_code >= centuries.size() ? 1900 : centuries[century_code];

		auto const result = std::chrono::year{century + in_century} / month / day;
		if (!result.ok()) return {};
		return result;
	}
