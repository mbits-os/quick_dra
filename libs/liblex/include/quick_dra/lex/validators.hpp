...
namespace quick_dra {
	static constexpr auto kInvalidChecksum = std::numeric_limits<unsigned short>::max();
	...
	namespace detail {
		...
		struct checksum_is_a_digit {
			static bool checksum_is_valid(unsigned short checksum, char tested) noexcept {
	...
	struct validator_suite : ChecksumPolicy, SelectorPolicy, ComparePolicy {
		...
		static bool is_valid(std::string_view id) noexcept {
			...
			return compare_t::checksum_is_valid(checksum, selector_t::select(id));
		}
	};

	using tax_id_validator = validator_suite<detail::tax_id, detail::select_last_character>;
	using social_id_validator = validator_suite<detail::social_id, detail::select_last_character>;
	using id_card_validator = validator_suite<detail::id_card, detail::select_nth_character<3>>;
	using pl_passport_validator = validator_suite<detail::pl_passport, detail::select_nth_character<2>>;
}  // namespace quick_dra
