...
def docker_args(version, env):
    ...
    args = [arg for pair in (['-e', f'{key}={_env[key]}']
                             for key in _env) for arg in pair]
    return ['docker', 'run', '--rm', *args,
            '-v' f'{__dir__}:/tmp/lint', f'github/super-linter:{version}']
...
for job_name, steps in JOBS:
    ...
    for name, version, env in steps:
        print(
            f'step: \033[1;49;96m{name}\033[m \033[2;49;32m(github/super-linter:\033[0;49;32m{version}\033[2;49;32m)\033[m')
